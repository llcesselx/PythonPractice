...
for y in range(rows):
  for x in range(cols):
    # As we iterate throught the loop, in this code block, the values of x and y will be: 
    # (0,0), (1,0), (2,0), (3,0) ...
    # (0,1), (1,1), (2,1), (3,1) ...
    # (0,2), (1,2), (2,2), (3,2) ...
    # ...
    # So the coordinates for the corners are (0,0), (cols-1,0), (0, rows-1), (cols-1, rows-1)
    
    if (x==0 and y == 0) or (x == cols-1 and y == 0) or (THIRD_CASE) or (FOURTH_CASE):
      # print a plus character (`+`) with NO NEWLINE afterwards
    elif HORIZONTAL_BORDER_CASES: 
      # print a hypthen character (`-`) with NO NEWLINE afterwards 
    elif VERICAL_BORDER_CASES: 
      # print a pipe character (`|`) with NO NEWLINE afterwards
    else:
      # print a space character (` `) with NO NEWLINE afterwards 
  # After the inner loops prints one entire line, print a newline
```
